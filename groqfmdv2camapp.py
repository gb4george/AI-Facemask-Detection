...
def load_model():
    ...
    Optimized for low-quality/low-light laptop webcams using tight bounds (conf=0.50, iou=0.45).
    """
...
def main():
    ...
    st.caption(
        ...
        "Applies real-time frame scaling, contrast adjustment, and strict confidence thresholds."
    )
    ...
    enable_enhancement = st.sidebar.checkbox(
        ...
        value=True,
        help="Applies CLAHE (Contrast Limited Adaptive Histogram Equalization) to brighten dark indoor rooms and improve contrast on low-quality webcams."
    )
    ...
    st.sidebar.markdown(
        ...
        **Countermeasures Active:**  
         ...
         - Frame resizing to 640px width to reduce latency  
         - Strict `conf=0.50` and `iou=0.45` filtering  
        """
    )
    ...
    with webcam_container:
        ...
        if run_camera:
            ...
            if not cap.isOpened():
                st.error("Could not open webcam. Check camera permissions or if another app is using it.")
                return
            ...
            with st.status("Processing low-res stream...", expanded=False) as status:
                ...
                while True:
                    ...
                    # Show current frame using updated width parameter instead of deprecated use_column_width
                    frame_placeholder.image(
                        annotated_rgb,
                        caption=f"Live Detections (conf ≥ {conf_thresh:.2f}, iou = {iou_thresh:.2f})",
                        width="stretch"
                    )
                    ...
                    # Check sidebar state to stop live detection cleanly
                    if not st.sidebar.checkbox("Start webcam", value=True):
                        break
